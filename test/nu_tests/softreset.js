...
exports.softResetOk_cb = {
    ...
    'soft reset should give back true': function(test) {
        test.expect(2);
        sens.softReset(function(err, val) {
            test.ifError(err);
            test.ok(val);
            test.done();
        });
    },
    'soft reset should set confRegTS to "now" ': function(test) {
        test.expect(3);
        var now_ = +new Date() - 1;
